...
public class CategorizatorTests {
    ...
    @Test
    public void testGetChildren_ShouldReturnCorrectResults() {
        Category categoryA = new Category("a","a","a");
        Category categoryB= new Category("b","b","b");
        Category categoryC= new Category("c","c","c");
        this.categorizator.addCategory(categoryA);
        this.categorizator.addCategory(categoryB);
        this.categorizator.addCategory(categoryC);
        this.categorizator.assignParent("b","a");
        this.categorizator.assignParent("c","b");
        Iterable<Category> childrenOfA = this.categorizator.getChildren("a");
        Iterable<Category> childrenOfB= this.categorizator.getChildren("b");
        Iterable<Category> parents = this.categorizator.getHierarchy("c");

        assertEquals(this.categorizator.size(), 3);
    }

    @Test
    public void testGetTop3Results_ShouldReturnCorrectResults() {
        Category categoryA = new Category("a","a","a");
        Category categoryB= new Category("b","b","b");
        Category categoryC= new Category("c","c","c");
        Category categoryD= new Category("d","d","d");
        this.categorizator.addCategory(categoryA);
        this.categorizator.addCategory(categoryB);
        this.categorizator.addCategory(categoryC);
        this.categorizator.addCategory(categoryD);
        this.categorizator.assignParent("b","a");
        this.categorizator.assignParent("c","b");
        this.categorizator.assignParent("d","c");

        Iterable<Category> top3CategoriesOrderedByDepthOfChildrenThenByName = this.categorizator.getTop3CategoriesOrderedByDepthOfChildrenThenByName();
        assertEquals(this.categorizator.size(),4 );
    }
}
