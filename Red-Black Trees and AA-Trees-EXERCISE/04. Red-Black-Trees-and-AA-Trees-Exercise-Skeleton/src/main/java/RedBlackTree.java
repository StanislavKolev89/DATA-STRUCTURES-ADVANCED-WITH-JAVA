...
public class RedBlackTree<Key extends Comparable<Key>, Value> {
    ...
    private Key select(Node x, int rank) {
        ...
        int nextSize = size(x.left);

        if (nextSize > rank) {
            return select(x.left, rank);
        } else if (nextSize < rank) {
            return select(x.right, rank - nextSize - 1);
        } else {
            return x.key;
        }
